...
After completing non-trivial code, config, dependency, schema, migration, or test changes and running the most relevant initial verification, explicitly ask the `reviewer` sub-agent to review only the task-specific diff and the main agent's verification results before the final response. If `reviewer` is unavailable, use `/review` when available. Skip this for trivial documentation/text-only changes, very small low-risk edits, or when `reviewer` and `/review` are unavailable; in those cases perform a focused self-review instead and mention it only when it affects confidence.

When asking `reviewer`, keep its context independent. Do not use full-history fork, `fork_context`, or equivalent context-inheritance options for review unless the user explicitly asks for that. Start `reviewer` with a concise, self-contained prompt that includes the task summary, the repository or working directory, the expected diff scope, and the verification already run. If the available tool rejects combining a reviewer role with context-forking options, retry without context forking.

Ask `reviewer` to use the task context to prioritize concrete correctness issues, behavior regressions, compatibility problems, safety risks, and missing or weak verification. The `reviewer` should not rerun tests or commands by default. It may recommend additional verification, and should run the narrowest relevant check only when existing verification is missing, failed, inconclusive, unavailable, or clearly insufficient for the changed behavior. Require findings to include severity, file/line references, rationale, and reproduction or verification steps when possible. "No findings" is an acceptable result.
