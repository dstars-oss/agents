...
This file defines Claude Code's default working style across all projects.
...
After completing non-trivial code, config, dependency, schema, migration, or test changes and running the most relevant initial verification, spawn an independent reviewer subagent via the Task tool to review only the task-specific diff and verification results before the final response. Use a concise, self-contained prompt and do not pass the full conversation history. If a subagent is not appropriate, use `/code-review` (or `/review` for a pull request) when available; otherwise perform a focused self-review for non-trivial changes.

Instruct the reviewer subagent to prioritize concrete correctness issues, behavior regressions, compatibility or safety risks, and missing verification. Fix only findings that are clearly valid and material, rerun the narrow relevant verification after any fix, and mention unresolved material findings, assumptions, or skipped verification in the final response.
